...
export const InputTask = () => {
    ...
    return (
        <>        
            <div className={`flex pt-3 w-full h-screen items-center tablet:gap-2 flex-col gap-4`}>
                {/* close button */}
                <div onClick={toggleSideBarFunction} className="self-end relative bottom-3 right-2 w-fit h-fit bg-red-500 p-1 rounded-full">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-x-lg" viewBox="0 0 16 16">
                        <path d="M2.146 2.854a.5.5 0 1 1 .708-.708L8 7.293l5.146-5.147a.5.5 0 0 1 .708.708L8.707 8l5.147 5.146a.5.5 0 0 1-.708.708L8 8.707l-5.146 5.147a.5.5 0 0 1-.708-.708L7.293 8z"/>
                    </svg>
                </div>
                <span className="text-lg desktopMini:text-2xl">Lista de Tarefas</span>
                {/* task input */}
                <div className="flex justify-center w-full items-center">
                    <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Digite uma tarefa..." className="placeholder:text-zinc-400 focus:border focus:border-blue-400 placeholder:text-xs w-4/5 shadow-md px-3 py-2 text-white font-normal rounded-md bg-zinc-700 "/>
                </div>
                {/* add button */}
                <div className="flex justify-center w-full items-center">
                    <button onClick={setElementStorage} className="bg-blue-500 flex items-center justify-center text-white font-medium w-4/5  tablet:w-fit tablet:px-4 desktop:py-2 desktopMini:px-6 h-fit py-2 px-9 shadow-md rounded-md">
                ...
                {/* delete button */}
                <div className="flex justify-center w-full items-center">
                    <button onClick={() => setDisplayControl({...displayControl, visible: 'flex', blur: true})} className="bg-red-500 desktop:py-2 flex items-center justify-center text-white font-medium w-4/5  desktopMini:px-6 tablet:w-fit tablet:px-4 h-fit py-2 px-9 shadow-md rounded-md">
